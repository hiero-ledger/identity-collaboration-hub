...
import { JSONUint8Array, KeplrError, Message, MessageRequester, Result } from '@keplr-wallet/router'
...
import { RNRouterBackground } from '../common'
...
export abstract class WalletConnectManager {
  ...
  public getClient(uri: string): WalletConnect | undefined {
    return this.clientMap.get(uri)
  }

  public async restoreClient(session: WalletConnect['session']) {
    ...
    if (client.connected) {
      ...
      client.on('call_request', (error: Error | null, payload: any) => {
  ...
  public isClientInitialized(uri: string): boolean {
    return this.clientMap.has(uri)
  }

  public isClientPending(uri: string): boolean {
    return this.pendingClientMap.has(uri)
  }

  public canInitClient(uri: string): boolean {
    return !this.isClientInitialized(uri) && !this.isClientPending(uri)
  }

  public async initClient(uri: string): Promise<WalletConnect> {
  ...
  protected readonly onCallRequest = async (client: WalletConnect, error: Error | null, payload: any) => {
    if (error) {
      console.log(error)
      return
    }
  ...
  protected abstract onSessionDisconnected(client: WalletConnect): Promise<void>
...
export class WalletConnectStore extends WalletConnectManager {
   ...
   */
  @observable
  protected _isPendingClientFromDeepLink = false

  @observable
  protected _needGoBackToBrowser = false
  ...
  public constructor(
     ...
     */
    this.eventListener.addEventListener('keplr_keystoreunlock', () => this.sendAccountMayChangedEventToClients())
    this.eventListener.addEventListener('keplr_keystorechange', () => this.sendAccountMayChangedEventToClients())
  }

  public get isPendingClientFromDeepLink(): boolean {
   ...
   */
  public get needGoBackToBrowser(): boolean {
  ...
  public onAndroidActivityKilled() {
   ...
   */
  @action
  public clearNeedGoBackToBrowser() {
  ...
  public getSession = computedFn((sessionId: string) => {
    return this.sessions.find((session) => session.key === sessionId)
  })

  @computed
  public get sessions(): WalletConnect['session'][] {
  ...
  protected async restore(): Promise<void> {
    const deepLinkClientMap = await this.kvStore.get<Record<string, true | undefined>>('deep_link_client_keys')
  ...
  public async disconnect(sessionId: string): Promise<void> {
    const client = this._clients.find((client) => client.session.key === sessionId)
    if (client) {
  ...
  protected async getPersistentSessions(): Promise<WalletConnect['session'][]> {
    const result = await this.kvStore.get<WalletConnect['session'][]>('persistent_session_v1')
    if (!result) {
      return []
    }
    return result
  ...
  protected async setPersistentSessions(value: WalletConnect['session'][]): Promise<void> {
    await this.kvStore.set('persistent_session_v1', value)
  }

  protected async saveDeepLinkClientKeyMap(keys: Record<string, true | undefined>): Promise<void> {
  ...
  protected async onSessionConnected(client: WalletConnect): Promise<void> {
    ...
    if (!clients.find((persistent) => persistent.session.key === client.session.key)) {
      runInAction(() => {
        clients.push(client)
      })
  ...
  protected async onSessionDisconnected(client: WalletConnect): Promise<void> {
    ...
    runInAction(() => {
      this._clients = this._clients.filter((persistent) => persistent.session.key !== client.session.key)
    })
    await this.setPersistentSessions(
    ...
    for (const chainInfo of this.chainStore.chainInfos) {
      await this.permissionStore
        ...
        .removeOrigin(WCMessageRequester.getVirtualSessionURL(client.session.key))
    }
  }
}

class WCMessageRequester implements MessageRequester {
  public constructor(
    protected readonly eventEmitter: EventEmitter,
    protected readonly sessionId: string
  ) {}

  public static getVirtualSessionURL = (sessionId: string): string => {
    return `https://keplr_wc_virtual.${sessionId}`
  }

  public static isVirtualSessionURL = (url: string): boolean => {
    return url.startsWith('https://keplr_wc_virtual.')
  }

  public static getSessionIdFromVirtualURL = (url: string): string => {
  ...
  public async sendMessage<M extends Message<unknown>>(
    port: string,
    msg: M
    ...
    if (result.error) {
      if (typeof result.error === 'string') {
        ...
        throw new KeplrError(result.error.module, result.error.code, result.error.message)
      }
    }
