...
import { RNMessageRequesterInternalToUI } from './RNMessageRequester'

export class RNEnv {
  public static readonly produceEnv = (sender: MessageSender): Env => {
    ...
    return {
      ...
      requestInteraction: async (_, msg) => {
        ...
        return await new RNMessageRequesterInternalToUI().sendMessage(APP_PORT, msg)
      },
    }
  }

  protected static readonly checkIsInternalMessage = (sender: MessageSender): boolean => {
    return sender.id === 'react-native' && sender.url === 'react-native://internal'
  }
}
